...
func _init() -> void:
	...
	print("temp | media div1 | media div5 | media div10 | attrs 95+ | aprendidas")
	...
	for j in _jugadores(piramide):
		for a in j["atributos"]:
			if int(j["atributos"][a]) >= 95:
				altos += 1
	print("%4d | %10.1f | %10.1f | %11.1f | %9d | %d" % [t, _media_division(piramide, 0),
		_media_division(piramide, 4), _media_division(piramide, 9), altos, nuevas])
