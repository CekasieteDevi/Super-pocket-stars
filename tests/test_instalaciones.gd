...
func _test_efectos_escalan_con_el_nivel(rng: RandomNumberGenerator) -> void:
	...
	equipo.instalaciones["medica"] = Instalaciones.NIVEL_MAXIMO
	equipo.instalaciones["estadio"] = Instalaciones.NIVEL_MAXIMO
	equipo.instalaciones["juveniles"] = Instalaciones.NIVEL_MAXIMO
	...
	if ok:
		print("OK: el nivel maximo reduce el riesgo de lesion y sube recuperacion/aforo/camada respecto a nivel 1.")
	else:
